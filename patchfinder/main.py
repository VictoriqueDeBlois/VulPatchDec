...
import configs
from load_data import NewCVEDataset
...
if __name__ == '__main__':
    version = '0714'
    CHECK_POINTS_PATH = f"./output_{version}/Checkpoints"

    ####### Load the data loaders
    configs.get_singapore_time()
    logging.info('1/4: start to prepare the dataset.')

    train_data = NewCVEDataset(configs.train_file)
    valid_data = NewCVEDataset(configs.valid_file)
    test_data = NewCVEDataset(configs.test_file)

    train_dataloader = DataLoader(dataset=train_data, shuffle=True, batch_size=16, num_workers=1)
    valid_dataloader = DataLoader(dataset=valid_data, batch_size=16, num_workers=1)
    test_dataloader = DataLoader(dataset=test_data, batch_size=8, num_workers=1)
    ...
    from evaluate import evaluate, save_metrics_to_csv

    recalls, avg_mrr, manual_efforts = evaluate(model, test_dataloader, k_values,
                                                data_path=f'./metrics/2025_predict_{version}.csv',
                                                rank_info_path=f'./metrics/2025_rank_info_{version}.csv',
    ...
    metrics_save_path = f'./metrics/2025_metrics_{version}.csv'
