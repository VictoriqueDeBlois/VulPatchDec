...
import configs
from load_data import CVEDataset, PreCVEDataset, NewCVEDataset
...
def evaluate(model, testing_loader, k_values,
             data_path,
             rank_info_path,
             reload_from_checkpoint=False, load_path_checkpoint=None):

    """Evaluate the model on the given test loader."""
    Path(data_path).parent.mkdir(parents=True, exist_ok=True)
    Path(rank_info_path).parent.mkdir(parents=True, exist_ok=True)
...
def evaluate_refactor(model, testing_loader, data_path):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(testing_loader):
            ...
            outputs = outputs.to('cpu').flatten().tolist()
            cves.extend(batch['cve'])
            logits.extend(outputs)
            labels.extend(batch['label'].tolist())

        df = pd.DataFrame({'cve': cves, 'logit': logits, 'label': labels})
...
if __name__ == "__main__":
    ...
    MODEL_PATH = f"./output_{version}/Checkpoints/final_model.pt"

    test_data = NewCVEDataset(configs.test_file)

    test_dataloader = DataLoader(test_data, batch_size=256, num_workers=1)

    model = torch.load(MODEL_PATH, weights_only=False)
    
    logging.info(f'Evaluating model at {MODEL_PATH}:')

    evaluate_refactor(model, test_dataloader, f'../../metrics/patchfinder_{version}_output.csv')
