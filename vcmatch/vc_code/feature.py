...
def get_feature(row, commit_info):
    commit = row['commit']
    reponame = row['repo']
    weblinks, bug, issue, cve, datetime, filepaths, funcs, addcnt, delcnt = commit_info[
        commit]
...
commit_df = pd.read_csv("../data/patchlikehood.csv")
...
vuln_df['desc_token_counter'] = vuln_df['desc_token_counter'].apply(eval)
...
mess_df['mess_token_counter'] = mess_df['mess_token_counter'].apply(eval)
...
# set PHP-project patchlikehood = 0.5
...
for reponame in repos:
    ...
    commit_info = readfile('../data/{}_commit_info'.format(reponame))
    ...
    data_df = pd.concat(l)
    # data_df.to_csv('../dataset/Dataset_5000_{}.csv'.format(reponame), index=False)
    ...
    for i in tqdm(range(epoch)):
        ...
        df['vuln_type_1'], df['vuln_type2'], df['vuln_type3'] = zip(*df.apply(
    ...
    data_df.to_csv(
        '../dataset/Dataset_5000_{}.csv'.format(reponame), index=False)
