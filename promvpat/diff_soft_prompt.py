...
logger = setup_logging('diff_soft_prompt', True)
...
def _load_data(df: pd.DataFrame):
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        ...
        label = row['label']
        idx = row['idx']
        input_example = InputExample(text_a=cve_description.lower(), text_b=commit_diff.lower(),
...
def load_dataset(args):
    ...
    df = df.rename(columns={'desc_cve': 'cve_description', 'diff_ori': 'commit_diff'})
...
def train_with_early_stopping(prompt_model, train_dataloader, validation_dataloader,
    ...
    for epoch in range(args.epoch):
        ...
        if val_acc > best_val_acc + min_delta:
            ...
            torch.save(prompt_model.state_dict(), f"../model/diff_soft_prompt/best_checkpoint.pth")
        else:
            patience_counter += 1
...
def setup_model_and_dataloaders(args):
    ...
    # 设置模板和数据加载器
    template_text =  'The CVE {"placeholder":"text_a"} is fixed by the code {"placeholder":"text_b"} {"mask"}.'
    mytemplate = SoftTemplate(model=plm, tokenizer=tokenizer, text=template_text, num_tokens=20)

    out_dir = Path('./diff_soft_prompt_temp_data')
    ...
    # 设置模型
    myverbalizer = SoftVerbalizer(tokenizer, plm, num_classes=2, label_words=[['true'], ['false']])
...
def main():
    ...
    Path('../model/diff_soft_prompt').mkdir(parents=True, exist_ok=True)
    # # 加载数据集
    ...
    # 保存模型
    torch.save(prompt_model, '../model/diff_soft_prompt/diff_soft_prompt.pth')
